...
<template>
  <td class="disease-td">
  <!--<div>-->
      <div v-for="(disease,index) in geneResp" v-if="disease.geneData.omims.length!==0" >
        <!--小圆点-->
        ...
        <span v-for="phenotypeMapSingle in omimSingle.phenotypeMaps">
          ...
          <span v-if="!phenotypeMapSingle.phenotype.inheritance" class="bold">[无]</span>
          ...
          <!--<a v-else="" class="po common-a" @click="showHPO(omimSingle.clinicalSynopsis)" data-toggle="tooltip" data-placement="bottom" :data-original-title='omimSingle.titles.preferred'>-->
            <!--{{omimSingle.titles.chinese?omimSingle.titles.chinese:omimSingle.titles.preferred}}-->
          <!--</a>-->
          <!--(100100)-->
...
<script>
  export default {
    ...
    watch:{
      geneResp:function (newData) {
        const _vue = this;
        $.each(newData,function (i,data) {
          _vue.allNull += data.geneData.omims.length
        })
      }
    },
    methods:{
