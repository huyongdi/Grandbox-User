...
<template>
  <div class="modal fade  bs-example-modal-lg" tabindex="-1" id="variation-d" role="dialog"
       aria-labelledby="gridSystemModalLabel3">
    <loading v-if="loadingV"></loading>
...
<script>
  export default {
    ...
    mounted: function () {
      const _vue = this;
      $('#variation-d').on('show.bs.modal', function (e) {
        _vue.myAxios({
          url: _vue.dbUrl+'database/snv/?query=' + this.locus,
        }).then(function (resp) {
          _vue.loading = false;
