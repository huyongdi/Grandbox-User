...
<template>
  <div>
    ...
    <div class="alert alert-danger alert-dismissible fade in noFound hide" role="alert" id="id_alert">
      <h4>警告：</h4>
      <p>以下基因名：（
        <span v-for="(single,index) in notfound">
                <a class="noFound-a common-a" :href="dbHtml +'#/gene?query=' + single" target="_blank">{{single}}</a>
                <span v-if="index !=notfound.length-1"> , </span>
        </span>
      ...
      <table class="table my-table no-shadow">
        ...
        <tr v-for="data in allData">
          ...
          <span v-for="(gene,index) in data.geneId">
            <a class="common-a" target="_blank" :href="dbHtml+'#/geneD?id=' + gene">{{gene}}</a>
        ...
        <tr v-if="allData.length == 0 && doneHttp3" style="text-align: center">
          ...
          <div class="modal-body">
            ...
            <table class="table my-table no-shadow">
              ...
              <tr v-for="data in tranData">
                ...
                <!--<td>-->
                  <!--<span v-for="(gene,index) in data.geneId">-->
                    <!--<a target="_blank" :href="dbHtml+'#/geneD?geneId=' + gene">{{gene}}</a>-->
                    <!--<span v-if="index == data.geneId.length">；</span>-->
                  <!--</span>-->
                <!--</td>-->
                <td>{{data.cov1 | percentData}}</td>
              ...
              <tr v-if="tranData.length == 0 && doneHttp4" style="text-align: center">
                <td colspan="8">暂无数据!</td>
              </tr>
              </tbody>
            </table>
