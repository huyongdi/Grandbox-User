<!--suppress HtmlUnknownTag -->
<template>
  <div class="row">
    <loading v-if="loading"></loading>
    <div class="done-list col-sm-11">
      <div class="title">
        <span class="title-b">任务详情</span>
        <span class="title-s">< 已完成</span>
      </div>
      <div class="btn-content">
        <span class="my-btn refresh" @click="refresh"><img src="../../static/img/red-refresh.png" alt="">数据刷新</span>
      </div>

      <!--<template>-->
        <!--<el-table :data="results" border stripe>-->
          <!--<el-table-column label="任务号">-->
            <!--<template scope="scope">-->
              <!--<span :data-type="scope.row.type">{{ scope.row.id}}</span>-->
            <!--</template>-->
          <!--</el-table-column>-->
          <!--<el-table-column label="软件/工具">-->
            <!--<template scope="scope">-->
              <!--{{scope.row.appInfo.name}}-->
            <!--</template>-->
          <!--</el-table-column>-->
          <!--<el-table-column label="样本编号">-->
            <!--<template scope="scope">-->
              <!--<span v-if="scope.row.parameter">{{scope.row.parameter.prefix}}</span>-->
            <!--</template>-->
          <!--</el-table-column>-->
          <!--<el-table-column label="姓名">-->
            <!--<div>1111</div>-->
            <!--<template scope="scope">-->
              <!--{{scope.row.nameArr[1]}}-->
            <!--</template>-->
          <!--</el-table-column>-->
          <!--<el-table-column label="数据">-->
            <!--<template scope="scope">-->
              <!--{{scope.row.parameterInfo.datafile.code}}-->
            <!--</template>-->
          <!--</el-table-column>-->
          <!--<el-table-column label="开始时间">-->
            <!--<template scope="scope">-->
              <!--{{scope.row.time.start}}-->
            <!--</template>-->
          <!--</el-table-column>-->
          <!--<el-table-column label="操作">-->
            <!--<template scope="scope">-->
              <!--<img src="../../static/img/delete.png" title="删除" class="remove-job" :data-id=scope.row.id @click.stop="removeTask">-->
            <!--</template>-->
          <!--</el-table-column>-->
        <!--</el-table>-->
      <!--</template>-->

      <!--<div class="my-table-content">-->
        <table id="done-table" class="bc-fff my-table">
          <thead>
          <tr>
            <th>任务号</th>
            <th>
              软件/工具
              <div class="img1">
                <div class="hide-content">
                  <img class="img2" src="../../static/img/th-1.png" alt="">
                  <ul>
                    <li @click="appClick()">请选择软件/工具</li>
                    <li v-for="single in selectArr" @click="appClick(single.code)">{{single.name}}</li>
                  </ul>
                </div>
              </div>
            </th>

            <th>样本编号</th>
            <th>姓名</th>
            <th>数据</th>
            <!--<th>状态</th>-->
            <th>开始时间</th>
            <th>操作</th>
          </tr>
          </thead>
          <tbody>
          <tr v-if="results.length == 0 && doneHttp">
            <td colspan="7" class="center">暂无数据</td>
          </tr>
          <tr v-for="(result,index) in results" :class="{'tr-bc':index%2}" @click="toResult(result.appInfo.code,result.id)" title="点击查看结果">
            <td>
              <span :data-type="result.type">{{ result.id}}</span>
            </td>
            <td><span v-if="result.appInfo">{{result.appInfo.name}}</span></td>
            <td><span v-if="result.parameter">{{result.parameter.prefix}}</span></td>
            <td>{{result.nameArr[1]}}</td>
            <td>{{result.parameterInfo.datafile.code}}</td>
            <!--<td>-->
            <!--<i v-if="result.status == 'completed' && !result.error" class="fa fa-check text-success"-->
            <!--title="已完成"></i>-->
            <!--<i v-else="" class="fa fa-bug text-danger" title="出错"></i>-->
            <!--</td>-->
            <td><span v-if="result.time">{{result.time.start}}</span></td>
            <td>
              <!--<router-link v-if="result.grandmgds" :to="{path:'/dataA/foo/sgResult',query:{id:result.typeId}}"-->
              <!--class="fa fa-folder-open text-success open-icon po" title="查看任务"></router-link>-->
              <!--<router-link v-if="result.grandmitos" :to="{path:'/dataA/foo/sgResult',query:{id:result.typeId}}"-->
              <!--class="fa fa-folder-open text-success open-icon po" title="查看任务"></router-link>-->
              <!--<i v-if="result.status == 'error'" class="fa fa-refresh po" title="重新运行"-->
              <!--:data-id=result.id></i>-->
              <img src="../../static/img/delete.png" title="删除" class="remove-job" :data-id=result.id
                   @click.stop="removeTask">
            </td>
          </tr>
          </tbody>
        </table>
      <!--</div>-->

      <page :childCount="count" :childReset="0" @childCurrent="getCurrent"></page>
    </div>
  </div>
</template>

<script>
  import page from './global/Page.vue'

  export default {
    components: {
      'page': page
    },
    data: function () {
      return {
        doneHttp:false,
        results: [],
        loading: false,
        count: 0,
        pageNum: 1,
        selectArr: [],
        appCode: ''
      }
    },
    created: function () {
      this.getList();
      this.getSelect();
      this.getSg();
    },

    methods: {
      getSg:function () {
        const _vue = this;
        this.myAxios({
          url:'application/job/sge/',
          method:'get',
        }).then(function (resp) {

        }).catch(function (error) {
          _vue.catchFun(error);
        });
      },
      appClick: function (code) {
        this.appCode = code ? code : '';
        this.pageNum = 1;
        this.getList()
      },
      getList: function () {
        this.loading = true;
        const _vue = this;
        this.doneHttp = false;
        this.myAxios({
          url: 'application/job/?status=completed&page=' + this.pageNum + '&app=' + this.appCode
        }).then(function (resp) {
          _vue.count = resp.data.count;
          $.each(resp.data.results, function (a, b) {
            b.time.start = b.time.start.substring(0, b.time.start.indexOf('T'));
            b.nameArr = [];
            $.each(b.name.split(':'), function (i, data) {
              b.nameArr.push(data)
            })
          });
          _vue.results = resp.data.results;
          _vue.doneHttp = true;
          _vue.loading = false;
        }).catch(function (error) {
          _vue.catchFun(error);
        })
      },
      getCurrent: function (data) {
        this.pageNum = data;
        this.getList();
      },
      refresh: function () {
        this.getList();
      },
      removeTask: function (event) {
        const _vue = this;
        if (confirm('确定要删除该任务吗？')) {
          const sId = $(event.target).data('id');
          this.myAxios({
            url: 'application/job/' + sId + '/',
            method: 'delete'
          }).then(function () {
            alert('删除成功');
            $(event.target).closest('tr').remove();
          }).catch(function (error) {
            _vue.catchFun(error);
          });
        }
      },
      getSelect: function () {
        const _vue = this;
        this.myAxios({
          url: 'application/app/',
        }).then(function (resp) {
          _vue.selectArr = resp.data.results;
        }).catch(function (error) {
          _vue.catchFun(error);
        })
      },
      toResult: function (code, id) {
        let url = '';
        if (code == 'grandmgd') {
          url = '/taskM/foo/sgResult'
        } else if (code == 'grandmito') {
          url = '/taskM/foo/mtResult'
        } else if (code == 'grandwcnv') {
          url = '/taskM/foo/cnvResult'
        } else if (code == 'grandanno') {
          url = '/taskM/foo/snvResult'
        } else if (code == 'grandtrio') {
          url = '/taskM/foo/trioResult'
        }
        this.$router.push({path: url, query: {id: id}})
      }
    }
  }
</script>

<style scoped lang="less">
  @border: rgb(185, 184, 184);
  .done-list {
    .btn-content {
      margin: 15px 0 0 0;
    }
    table#done-table {
      overflow: auto;
      .remove-job {
        margin: -3px 0 0 10px;
        cursor: pointer;
      }
    }
  }
</style>
